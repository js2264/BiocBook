...
4. *Versionize*: automatically generate specific website versions and Docker images for individual [Bioconductor releases](https://contributions.bioconductor.org/use-devel.html). 

::: {.callout-tip icon='true'}
## A `BiocBook` package in Bioconductor release `X.Y` (i.e. with a branch named `RELEASE_X_Y`) provides a **Docker image** and a **`BiocBook` website**, both built against this specific Bioconductor release `X.Y`.
:::
...
Any other branch name (e.g. `main`, `hotfix-...`, etc.) can of course be used, but won't 
